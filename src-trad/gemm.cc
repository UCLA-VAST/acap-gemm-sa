...
Gemm<DT, R, TM, TK, TN, MM, KK, NN, RFWD, CFWD>::
     ...
     adf::output_async_buffer<DT> & __restrict oout)
{
  ...
  auto flush_step = [&]() {
    ...
    for (int i=0; i<TM*TN/VLEN; i++) {
      ...
      //        row, col, lap, stop_lap, iter, stop_iter, flush, i, vec2str(*in).c_str());
      *out++ = *in++;
    }
  ...
  for (lap=0; lap<stop_lap; lap++) {
    ...
    for (iter=0; iter<stop_iter; iter++) {
      ...
      if constexpr (RFWD) {
        rout.acquire();
        auto rin_ = aie::begin_vector<VLEN>(rin.data());
        auto rout_ = aie::begin_vector<VLEN>(rout.data());
        for (int i=0; i<TM*TK/VLEN; i++) { *rout_++ = *rin_++; };
        rout.release();
      }
      ...
      if constexpr (CFWD) {
        cout.acquire();
        auto cin_ = aie::begin_vector<VLEN>(cin.data());
        auto cout_ = aie::begin_vector<VLEN>(cout.data());
        for (int i=0; i<TM*TK/VLEN; i++) { *cout_++ = *cin_++; };
        cout.release();
      }
    ...
    //       row, col, lap+1, stop_lap, iter, stop_iter, oout.data());
    oout.acquire();
    ...
    for (int i=0; i<TM*TN/VLEN; i++) {
      ...
      //        row, col, lap+1, stop_lap, iter, stop_iter, i, vec2str(*in).c_str());
      *out++ = *in++;
    }
    ...
    //STDERR("id=(%d,%d) lap=%d/%d iter=%d/%d end\n", row, col, lap+1, stop_lap, iter, stop_iter);
